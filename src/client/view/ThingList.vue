<template>
  <content-module name="things">
    ...
    <div>
      <el-card class="box-card" v-for="thing in things" :key="thing.id">
    ...
    <el-dialog :title="form.id ? $t('thing.edit.update') : $t('thing.edit.create')" v-model="formVisible">
  ...
  methods: {
    ...
    saveForm () {
      this.$refs.thing.validate(valid => {
        if (valid) {
          let promise
          if (this.form.id) {
            promise = thingRes.update({ id: this.form.id }, this.form)
          } else {
          ...
          promise.then(() => {
            ...
            this.$message({
              type: 'success',
              message: this.form.id ? this.$t('message.updated') : this.$t('message.created')
            })
            this.fetch()
          }).catch(() => {})
        }
    ...
    deleteThing (thing) {
      this.$confirm(`This action will remove the selected thing: ${thing.name} forever, still going on?`, this.$t('confirm.title'), {
        ...
        thingRes.delete({ id: thing.id }).then(() => {
          this.$message({
            type: 'success',
